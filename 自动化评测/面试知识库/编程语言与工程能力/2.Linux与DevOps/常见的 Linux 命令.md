...
## 脚本运行
在 Linux 中运行 `.sh` 脚本有多种方式，下面为你介绍常见的几种方法。
...
- 若脚本位于当前目录，要使用 `./` 来指定路径，避免系统将其误认为是环境变量中的命令，具体命令为：
...
- 若脚本位于系统路径（像 `/usr/bin`）之中，可直接输入脚本名称运行：
...
这种方式无需给脚本添加执行权限，直接借助解释器来运行脚本，命令如下：
...
### **4. 使用 source 命令（用于加载脚本）**

当你需要在当前 shell 环境中执行脚本（比如加载环境变量）时，可使用 `source` 命令，具体为：
...
1. **文件路径问题**
    - 若脚本不在当前目录，要使用绝对路径或者相对路径。
    - 示例：`/home/user/scripts/script.sh` 或者 `../scripts/script.sh`。
2. **文件格式问题**
    - 要保证脚本是 Unix/Linux 格式（LF 换行符），Windows 格式（CRLF）可能会引发错误。
    - 你可以使用 `dos2unix` 工具进行转换：
...
由于您无法使用conda，我们可以使用pip来在指定的Python虚拟环境中安装库。以下是具体步骤：

1. 直接使用虚拟环境中的pip来安装包：

```bash
C:\ProgramData\anaconda3\envs\mcp\python.exe -m pip install 包名
```

例如，如果要安装numpy包：
```bash
C:\ProgramData\anaconda3\envs\mcp\python.exe -m pip install numpy
```

2. 如果需要安装特定版本的包：
```bash
C:\ProgramData\anaconda3\envs\mcp\python.exe -m pip install 包名==版本号
```

3. 如果需要从requirements.txt文件安装多个包：
```bash
C:\ProgramData\anaconda3\envs\mcp\python.exe -m pip install -r requirements.txt
```

4. 如果遇到网络问题，可以使用国内镜像源：
```bash
C:\ProgramData\anaconda3\envs\mcp\python.exe -m pip install 包名 -i https://pypi.tuna.tsinghua.edu.cn/simple
```

注意事项：
- 确保使用完整的Python解释器路径，这样可以确保包被安装到正确的虚拟环境中
- 如果路径中包含空格，需要用引号将整个路径括起来
- 建议在安装包之前先升级pip：
```bash
C:\ProgramData\anaconda3\envs\mcp\python.exe -m pip install --upgrade pip
```
